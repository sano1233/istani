...
export type GalleryItem = ExternalImage & { id?: string };
...
export async function loadGalleryItems({
  query = 'gym fitness workout strength male training',
  pexelsCount = 24,
  unsplashCount = 24,
  supabaseLimit = 60,
}: LoadGalleryItemsOptions = {}): Promise<GalleryItem[]> {
  const [pexels, unsplash] = await Promise.all([
    fetchPexelsFitness(query, pexelsCount).catch(() => []),
    fetchUnsplashFitness(query, unsplashCount).catch(() => []),
  ]);

  const remote: GalleryItem[] = [...pexels, ...unsplash];
