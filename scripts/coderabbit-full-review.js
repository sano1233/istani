  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.includes('console.log') && !trimmed.includes('eslint-disable')) {
      suggestions.push({
        ...
        type: 'console-log',
        message:
          'Consider removing console.log or adding eslint-disable comment for production code',
    ...
    if (
      trimmed.match(/['"](https?:\/\/|api\/|localhost)/) &&
...
function main() {
  ...
  fs.writeFileSync('coderabbit-suggestions.json', JSON.stringify(output, null, 2));
