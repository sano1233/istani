...
async function validateDependencies() {
  ...
  try {
    ...
    try {
      ...
      recordTest('Dependencies installed', false, 'node_modules not found - run npm install', 'warning');
    }
    ...
    try {
      ...
      recordTest('AI Brain dependencies installed', false, 'ai-brain/node_modules not found', 'warning');
    }
  } catch (error) {
...
async function validateGitHubActions() {
  ...
  try {
    ...
    const content = await fs.readFile('.github/workflows/automated-code-resolver-merger.yml', 'utf8');
...
function generateReport() {
  ...
  log(`   Warnings: ${validationResults.warnings}`, validationResults.warnings > 0 ? 'yellow' : 'gray');
  
  if (total > 0) {
    const passRate = Math.round((validationResults.passed / total) * 100);
    log(`\n   Pass rate: ${passRate}%`, passRate === 100 ? 'green' : passRate >= 80 ? 'yellow' : 'red');
  }
