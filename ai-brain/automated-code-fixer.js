  ...
  try {
    ...
    try {
      const { stdout, stderr } = await execAsync(
        `npx eslint ${target} ${extensions} --format json`,
      );
    ...
    if (stdout) {
      try {
        ...
        if (total > 0) {
          ...
          const { stdout: afterStdout } = await execAsync('npm audit --json').catch((e) => ({
            stdout: e.stdout,
          }));
          if (afterStdout) {
            const afterAudit = JSON.parse(afterStdout);
            const afterVulns = afterAudit.metadata?.vulnerabilities || {};
            const remaining = Object.values(afterVulns).reduce(
...
async function checkSyntaxErrors(filePath = null) {
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `find ${target} -name "*.js" -o -name "*.mjs" -type f`,
    ).catch(() => ({ stdout: '' }));
...
async function main() {
  ...
  log(
    `\n⚙️  Mode: ${checkOnly ? 'Check Only (No Fixes Applied)' : 'Fix Mode (Applying Fixes)'}`,
    'cyan',
  );
