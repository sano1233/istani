...
async function postReviewComment(prNumber, reviews, approvals, requiredApprovals) {
  try {
    ...
    const consensusStatus = approvals.length >= requiredApprovals 
...
async function checkMergeReadiness(prDetails) {
  ...
  log(`   Mergeable: ${checks.mergeable ? '✅' : '❌'} ${prDetails.mergeable || 'UNKNOWN'}`, checks.mergeable ? 'green' : 'yellow');
...
async function processPR(prNumber, options = {}) {
  ...
  log(`\n👍 Approving PR...`, 'cyan');
  try {
    await execAsync(`gh pr review ${prNumber} --approve --body "Approved by AI Brain consensus (${approvals.join(', ')})"`);
